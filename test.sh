...
docoruntests() {
    docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=1001 $@ odoo runtests
}
# run 'runmigration' in the container
# extra arguments are passed to the 'run' command (example: -e FOO=bar is added to the list of args)
docorunmigration() {
    docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=1001 $@ odoo runmigration
}
docodown() {
    docker-compose -f test-compose.yml down
}
docoruncmd() {
    docker-compose -f test-compose.yml run --rm -e LOCAL_USER_ID=1001 $@
}
